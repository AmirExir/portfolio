...
The hero also has an independent 26-second event sequence: normal flow, a localized lightning strike, breaker opening, line isolation and load outage, ordered reclosing, and restored supply. The model depicts one radial transmission supply with a temporary fault cleared before a deliberately successful reclose. It does not represent an alternate downstream feed or UPS; the source-side battery remains upstream of the opened breakers and cannot supply the isolated loads. These assumptions explain the pictured outage, not a universal response to lightning. The times are presentation timing, not relay delays, reclose settings, or protection-study results.

Sending and receiving terminals each have three separate breaker poles. The line is de-energized while isolated; sending contacts close before receiving contacts, and downstream illumination ramps back only after both terminals close. Load windows, receiving-transformer effects, and downstream transfer packets are suppressed during outage while upstream generation/storage artwork continues. Equipment silhouettes remain visible. The strike is one short, deterministic local pulse, without a full-canvas flash or repeated strobing.

`PowerJourney.getEventState()` supplies the stage, contact positions, illustrative power/flash factors, and replay availability. Its `labels`, `eventCues`, and `cycleDuration` exports keep controller labels and renderer stages synchronized. `draw()` accepts separate ambient `time` and `eventTime` values, so replaying the event does not reset generation/storage motion. `reducedMotion` freezes ambient artwork and removes lightning; `suppressFlash` removes only the bolt, preserving the paused equipment orientation.

The native `[data-power-replay]` control offers “Replay strike” during motion and is disabled while its sequence is active. When paused or using reduced motion it instead toggles “Show outage” and “Restore power,” displaying static isolated/restored states without resuming animation. Its phase text uses `aria-live="off"`; only deliberate actions update `[data-power-announcement]`. The event clock advances only for visible power mode. AI modes and renderer fallbacks hide the control. Mode crossfades suppress lightning, and a motion-preference change immediately repaints away an existing bolt even if the hero is offscreen. An interrupted bolt stays suppressed until its strike phase ends, preventing repeated Pause/Play actions from revealing the same bright frame again. Engineering assumptions stay in this documentation; the hero retains concise stage labels.
...
That test instruments private helpers only inside its VM copy; no test-only helpers are exposed by the deployed renderer. It verifies that the desktop, mobile, and narrow-phone layouts construct all six technologies with distinct shapes after removing translation, scale, color, and captions. It also checks separate AC terminals and DC pairs, storage-state boundaries, actual packet/trail reversal, AC/DC transfer ordering, and idle behavior. The ordering test observes pulses on the real model segments rather than repeating the renderer's schedule formula.

Event checks cover stage boundaries and replay lockout, actual contact gaps and reclose order, suppression of downstream packets/window glow/base emission/transformer effects, and preserved upstream motion. The bolt must be deterministic and localized, with one bounded pulse per automatic cycle, no lightning under reduced motion, and no ambient reset when only flashes are suppressed. Browser checks cover replay cue selection, static outage/restoration inspection while paused or reduced, control visibility/fallbacks, retained flash suppression across repeated Pause/Play until the strike ends, and immediate flash removal when reduced motion is enabled with an already-painted strike offscreen. These tests cover illustrative geometry and animation logic, not engineering calculations. They complement visual inspection and do not certify equipment designs or validate a power-system model.
